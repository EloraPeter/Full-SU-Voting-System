...
async function createContestCategory(req, res) {
  try {
    ...
    if (!title) {
      return res
        .status(400)
        .json({ error: "Contest Category Title is required." });
    }
...
async function addUserToContest(req, res) {
  try {
    ...
    if (!user_id || !category_id) {
      return res
        .status(400)
        .json({ error: "user_id and category_id are required" });
    }
    ...
    if (existingContest.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "This user is already a contestant in that category" });
    }
...
async function listContestResult(req, res) {
  ...
  LEFT JOIN users uv ON uv.id = v.voter_id
  GROUP BY con.id
...
async function getCategoryContestResultVisual(req, res) {
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "No contestants found for this category" });
    }
